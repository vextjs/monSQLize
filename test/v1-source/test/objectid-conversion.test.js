...
function getConnectConfig() {
    const sharedUri = process.env.MONSQLIZE_SYSTEM_MONGO_URI;
    if (sharedUri) {
        return { uri: sharedUri };
    }
    return { useMemoryServer: true };
}
...
describe('自动 ObjectId 转换功能测试', function() {
    ...
    describe('事务场景', function() {
        ...
        before(async function() {
            txMsq = new MonSQLize({
                type: 'mongodb',
                databaseName: 'test_objectid_tx',
                config: getConnectConfig(),
            });
            const { collection } = await txMsq.connect();
            txMsq.collection = collection;
            const client = txMsq._adapter.client;
            txDb = client.db('test_objectid_tx');
        });

        after(async function() {
            if (txDb) {
                try { await txDb.collection('tx_users').deleteMany({}); } catch {}
            }
            if (txMsq) {
                await txMsq.close();
                txMsq = null;
        ...
        it('事务中的 ObjectId 转换', async function() {
            ...
            try {
                await session.withTransaction(async () => {
                    // 在事务中插入文档（managerId 存储为字符串）
                    await txDb.collection('tx_users').insertOne({
                        _id: userId,
                        name: 'Tx User',
                        managerId: managerId.toString(),
                    }, { session });
                ...
                // 事务提交后，用 msq.collection（自动 ObjectId 转换）查询
                const result = await txMsq.collection('tx_users').findOne({
                    _id: userId.toString(), // 传字符串，自动转为 ObjectId
                });

                assert.ok(result, '事务提交后应该能找到文档');
                assert.strictEqual(result._id.toString(), userId.toString(), '_id 应该匹配');
                assert.strictEqual(result.name, 'Tx User', '名称应该匹配');
            } finally {
