...
describe('MultiLevelCache - 分布式失效集成', () => {
    ...
    describe('分布式场景模拟', () => {
        ...
        it('应该在实例间同步缓存失效', async () => {
            ...
            // 实例 B 的缓存应该也被失效
            expect(await cacheA.local.get('user:1')).to.not.exist;
            expect(await cacheB.local.get('user:1')).to.not.exist;
        });
        ...
        it('应该支持模式匹配失效', async () => {
            ...
            expect(await cacheA.local.get('user:1')).to.not.exist;
            expect(await cacheA.local.get('user:2')).to.not.exist;
            expect(await cacheA.local.get('product:1')).to.exist;

            expect(await cacheB.local.get('user:1')).to.not.exist;
            expect(await cacheB.local.get('user:2')).to.not.exist;
        ...
        it('应该处理并发失效', async () => {
            ...
            // 两边都应该被清空
            expect(await cacheA.local.get('data:1')).to.not.exist;
            expect(await cacheB.local.get('data:1')).to.not.exist;
        });
    });
