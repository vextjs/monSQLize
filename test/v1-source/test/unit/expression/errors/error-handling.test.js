...
describe('⚠️ Error Handling Tests', function() {
  ...
  describe('1️⃣ Syntax Error Handling', function() {
    ...
    it('1.1 should handle invalid operator gracefully', function() {
      try {
        const result = expr("INVALID_FUNC(name)");
        // 如果到这里说明没有抛出错误，表达式对象已创建
        // 真正的错误应该在编译时抛出
        assert(result.__expr__, '表达式对象应该被创建');
      } catch (err) {
        // 如果在创建时就抛出错误也是可以的
    ...
    it('1.2 should handle mismatched parentheses', function() {
      try {
        const result = expr("CONCAT(name, age");  // 缺少右括号
        assert(result.__expr__, '语法错误可能在编译时才检测');
      } catch (err) {
    ...
    it('1.3 should handle unclosed string literal', function() {
      try {
        const result = expr("CONCAT(name, 'test)");  // 字符串未闭合
        assert(result.__expr__, '语法错误可能在编译时才检测');
      } catch (err) {
    ...
    it('1.4 should handle invalid comparison operator', function() {
      try {
        const result = expr("age == 25");  // 应该用 === 而不是 ==
        // 注意：== 可能被当作两个连续的 = 处理，或者被编译器拒绝
        assert(result.__expr__, '表达式对象可能被创建');
      } catch (err) {
        // 如果抛出错误也是合理的
    ...
    it('1.5 should handle missing operator', function() {
      try {
        const result = expr("name value");  // 缺少操作符
        assert(result.__expr__, '语法错误可能在编译时检测');
      } catch (err) {
    ...
    it('1.6 should handle invalid field reference', function() {
      try {
        const result = expr("$invalid.field.name");  // 字段引用不应该有 $
        // 这可能被解析为字符串 "$invalid"
        assert(result.__expr__, '可能被解析为字面量');
      } catch (err) {
        // 如果抛出错误也合理
  ...
  describe('3️⃣ Compilation Error Handling', function() {
    ...
    it('3.2 should handle circular reference (if detectable)', function() {
      ...
      // 这个测试主要检查编译器的健壮性
      try {
        const result = expr("value + value + value");
        assert(result.__expr__, '正常表达式应该成功');
      } catch (err) {
    ...
    it('3.3 should handle missing function arguments', function() {
      try {
        const result = expr("CONCAT()");  // CONCAT 需要至少 1 个参数
        assert(result.__expr__, '参数错误可能在编译时检测');
      } catch (err) {
    ...
    it('3.4 should handle too many function arguments', function() {
      try {
        // ABS 只接受 1 个参数，但传入多个
        const result = expr("ABS(value, age, name)");
        assert(result.__expr__, '参数数量错误可能在编译时检测');
      } catch (err) {
    ...
    it('3.6 should handle invalid lambda expression', function() {
      try {
        // Lambda 表达式格式错误
        const result = expr("FILTER(tags, invalid syntax)");
        assert(result.__expr__, 'Lambda 错误可能在编译时检测');
      } catch (err) {
    ...
    it('3.7 should handle invalid conditional expression', function() {
      try {
        // 三元运算符格式错误
        const result = expr("age > 30 ? 'old'");  // 缺少 : 和 else 部分
        assert(result.__expr__, '条件表达式错误可能在编译时检测');
      } catch (err) {
    ...
    it('3.9 should handle invalid SWITCH expression', function() {
      try {
        ...
        const result = expr("SWITCH(age > 30, 'old', age > 20)");  // 缺少 default
        assert(result.__expr__, 'SWITCH 错误可能在编译时检测');
      } catch (err) {
  ...
  describe('4️⃣ Error Recovery', function() {
    ...
    it('4.2 should provide helpful error messages', function() {
      try {
        const result = expr("CONCTA(name)");  // 拼写错误 CONCAT -> CONCTA
        assert(result.__expr__, '表达式可能被创建');
      } catch (err) {
        // 如果抛出错误，应该有有用的提示
        assert(err.message, '应该有错误消息');
        // 理想情况下应该建议正确的拼写
