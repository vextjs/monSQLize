...
/**
 * 示例 1: 基本表达式创建
 * $ 函数返回 ExpressionObject 类型
 */
function example1_basicExpression() {
    // 创建简单表达式
    const expr1 = $("age > 18");
    const expr2 = $("name === 'John'");
    const expr3 = $("value * 2");
    // 验证返回类型
...
async function example2_1_matchStage() {
    ...
    try {
        ...
        const result2 = await collection('users').aggregate([
            { $match: $("age > 18 && status === 'active'") }
        ]);
        // 三元运算符
        const result3 = await collection('users').aggregate([
            { $match: $("(age > 18 && age < 65) || vip === true") }
        ]);
...
function example3_1_nestedFunctions() {
    // 字符串操作嵌套
    const expr1 = $("UPPER(CONCAT(LOWER(TRIM(name)), ' - ', status))");
    // 数学函数嵌套
    const expr2 = $("SQRT(ABS(value)) + POW(base, 2)");
    // 条件嵌套
    const expr3 = $("age > 18 ? (age > 65 ? 'senior' : 'adult') : 'minor'");
...
function example3_2_lambdaExpressions() {
    // FILTER: 过滤数组
    const filterExpr = $("FILTER(tags, tag, tag.active === true)");
    // MAP: 映射数组
    const mapExpr = $("MAP(items, item, item.price * item.quantity)");
...
function example3_3_dateOperations() {
    const expr1 = $("YEAR(createdAt) === 2024");
    const expr2 = $("MONTH(createdAt) > 6 && DAY_OF_MONTH(createdAt) < 15");
    const expr3 = $("HOUR(timestamp) >= 9 && HOUR(timestamp) < 17");
...
function example4_1_typeAnnotations() {
    ...
    const expr = $("age > 18");
    // 属性访问（类型安全）
    const exprString = expr.__expr__;
    const compiled = expr.__compiled__;
    console.log('✅ 示例4.1: 类型注解验证成功');
    console.log(`  表达式: ${exprString}`);
    console.log(`  已编译: ${compiled}`);
    return { expr, exprString, compiled };
}
/**
 * 示例 4.2: 类型推断
 */
function example4_2_typeInference() {
    // TypeScript 自动推断类型
    const expr = $("value > 100");
    // 类型守卫
    if (expr && typeof expr === 'object' && '__expr__' in expr) {
        ...
        const result = await collection('products').aggregate([
            ...
            // 阶段2: 投影计算
            {
                $project: {
                    ...
                    finalPrice: $("price * (1 - discount) * (1 - tax)"),
                    priceLevel: $("price > 1000 ? 'expensive' : price > 500 ? 'medium' : 'cheap'")
                }
            },
            // 阶段3: 按分类分组
            {
