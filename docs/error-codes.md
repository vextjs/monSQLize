...
| 错误码 | 常见来源 | 优先检查 |
|--------|----------|----------|
| `INVALID_CONFIG` | 构造配置、连接池配置、SSH 配置、Redis/分布式能力配置 | 必填字段、字段类型、URI 前缀、认证字段、Redis/SSH 运行时配置 |
| `CONNECTION_FAILED` | MongoDB 连接或内存 MongoDB 启动失败 | MongoDB 地址、网络、认证、服务状态、`error.cause` |
| `NOT_CONNECTED` | `connect()` 前访问集合、Model、SSH tunnel address | 先 `await db.connect()`，确认连接生命周期 |
