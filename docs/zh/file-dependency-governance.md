...
| 依赖 | 策略 | 原因 |
|------|------|------|
| `cache-hub` | 精确版本 `2.2.4` | 上游 npm latest 已发布稳定版本并通过兼容验证；根包 direct dependency 固定到 2.2.4，`schema-dsl@2.1.3` 声明的传递依赖同为 2.2.4 |
| `schema-dsl` | 精确版本 `2.1.3` | 本次发布链路目标为当前非 deprecated 的 TypeScript 正式线；monSQLize 依赖 schema-dsl runtime 隔离能力负责 Model schema 解析、验证、自定义类型和消息，并以 2.1.3 执行 type-check 与 Model schema 验证 |

> `schema-dsl@2.1.3` 是本次发布链路的目标版本；npm 上历史 `2.3.x` 已标记为误发布 / deprecated，不得跟随升级。
...
- 根包 direct `cache-hub` 固定为 `2.2.4`；`schema-dsl@2.1.3` 声明的 `cache-hub` 版本同为 `2.2.4`，不需要额外 override。
- `schema-dsl` 固定为 `2.1.3`。
...
依赖治理基线已将 `schema-dsl` 从历史 `^1.2.5` 升级并固定到 `2.1.3`，验证标准如下：

1. 上游在 npm 发布了**非 deprecated** 的 2.x 目标版本：`2.1.3`。
2. `npm install schema-dsl@2.1.3 --save-exact` 后通过 `npm run type-check`。
...
6. 本文件、Profile、CHANGELOG 与 lockfile 必须同步到 `2.1.3`。
...
2. 根包 direct dependency 解析到 `2.2.4`；`schema-dsl@2.1.3` 也声明 `cache-hub@2.2.4`。
