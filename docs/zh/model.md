...
monSQLize 通过默认运行时依赖 `schema-dsl` 验证 Model 文档。安装 `monsqlize` 会自动安装 `schema-dsl`，每个已连接的 `MonSQLize` 运行时都会创建或接收一个隔离的 `schema-dsl/runtime` 实例，可通过 `schemaDsl` 选项配置。

`Model.define()` 只把定义保存到进程级 registry。schema 回调会在 `msq.model(name)` 把定义绑定到具体运行时时编译。只要 Model 定义了 schema，且没有通过 `options.validate: false`、单次操作 `skipValidation: true` 或运行时 `schemaDsl: false` / `{ enabled: false }` 显式关闭，验证就会执行。
...
// 绑定到运行时且定义了 schema 的 Model 会自动执行验证。
await User.insertOne({
    username: 'john',
...
- **验证开销**: 在典型 schema 较重的写入路径中，插入耗时约增加 5-10%
