...
- **不受 MongoDB 单事务时间限制**：步骤不运行在同一个 MongoDB 事务中。
- **步骤编排兼容**：既有代码仍可通过 legacy API 协调多个本地操作。
- **自动补偿尝试**：后续步骤失败时，会对已完成步骤逆序执行补偿。
- **定义元数据共享**：Redis 存储可在进程间共享 Saga 定义元数据。
- **应用侧负责持久化**：执行状态持久化、恢复和对账应放在应用/框架层。

> **持久化边界**：Redis 模式当前只持久化 Saga 定义元数据，例如 Saga 名称和步骤元数据。执行状态仍保留在进程内：已完成步骤、步骤结果、补偿进度和执行中的上下文不会写入 Redis。如果进程在步骤之间退出，monSQLize 不能自动恢复这次 Saga 执行。支付、订单、履约等需要崩溃恢复的流程，应在业务侧配合 durable journal/outbox、幂等键和对账机制。
...
Redis 共享不等于执行状态持久化。每个进程仍在内存中执行和补偿当前 Saga run；Redis 仅共享定义元数据。
