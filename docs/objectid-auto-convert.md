...
const msq = new MonSQLize({
    ...
    autoConvertObjectId: {
        // 自定义字段匹配模式
        customFieldPatterns: [
            /^ref/,                    // ref 开头：refUser, refPost
            /Reference$/,              // Reference 结尾：userReference
            /^parent/,                 // parent 开头：parentId, parentNode
            /^child/                   // child 开头：childId, childNode
        ]
    }
});
...
// 方案 2: 手动判断
const query = {
    externalId: isObjectId(externalId) 
