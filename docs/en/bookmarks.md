...
- ✅ **Global Operation**: Do not pass `keyDims` to operate all bookmarks (applicable to global reset)
- ✅ **Failed Detection**: `prewarmBookmarks` automatically detects out-of-range pages
- ✅ **Cache Availability Check**: All APIs throw `CACHE_UNAVAILABLE` error when cache is not available

## Usage scenarios

1. **System startup preheating** - Preheat hot pages to reduce first query delay
2. **Operation and Maintenance Monitoring** - View cached page distribution
3. **Clear cache after data changes** - Ensure the latest data is queried
4. **Memory Management** - Clean cache to release resources on demand
...
Warm up the bookmark cache of the specified page.
...
console.log('Preheating successful:', result.warmed);  // 5
console.log('Preheating failed:', result.failed);  // 0
...
try {
  ...
  if (result.failed > 0) {
    console.warn(`There is ${result.failed} page warm-up failed (may be out of data range)`);
  }
} catch (error) {
  if (error.code === 'CACHE_UNAVAILABLE') {
    console.error('Cache is unavailable, unable to warm up bookmark');
  } else {
    console.error('Preheating failed:', error.message);
  }
}
```
...
console.log('All bookmarks have been cleared:', clearAllResult.cleared, 'a');
```
...
```javascript
//Clear related caches after updating products in batches
async function updateProducts(updates) {
  ...
  //2. Clear bookmarks of related queries
  ...
  //3. Optional: Clear all relevant caches
  await collection('products').invalidate('findPage');

  console.log('✅ Product update completed, cache has been cleared');
}
```
...
async function prewarmOnStartup() {
  ...
  console.log('🚀 The system starts and starts to warm up bookmark...');

  //Preheat popular product query (first 10 pages)
  ...
  console.log(`✅ Hot Products: Warm up ${hotProducts.warmed} page`);
  ...
  console.log(`✅ Pending orders: Warm up ${recentOrders.warmed} page`);
  console.log('🎉 Preheating completed!');
}
...
## Scenario 2: Clean expired cache regularly
...
async function cleanupExpiredBookmarks() {
  ...
  console.log('🧹 Start cleaning up expired bookmarks...');
  ...
  console.log(`Query of cleared and discontinued products: ${cleared1.cleared} items`);
  ...
  console.log(`✅ Released ${before.count - after.count} caches`);
}
...
## Scenario 3: Refresh cache after data update
...
async function refreshCacheAfterUpdate(category, updates) {
  ...
  console.log(`🔄 Update ${category} category products...`);
  ...
  //3. Warm up new cache (first 5 pages)
...
//❌ Bad: Frequent cleaning of all caches
setInterval(async () => {
...
async function checkCacheHealth() {
  ...
  if (all.count > 10000) {
    console.warn('⚠️ There are too many Bookmark caches, it is recommended to clean them');
  }

  if (all.count === 0) {
    console.info('ℹ️ No bookmark cache, consider preheating popular queries');
  }
}
```
...
1. **Cache key matching**: `sort` of `keyDims` will automatically apply `ensureStableSort` normalization to ensure that the same cache key is used as findPage
...
try {
  ...
  if (result.failed > 0) {
    console.warn(`There is ${result.failed} page warm-up failed`);
  }
} catch (error) {
  if (error.code === 'CACHE_UNAVAILABLE') {
...
- [bookmarks sample code](../../examples/docs/bookmarks.ts)
