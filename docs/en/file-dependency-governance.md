...
| `schema-dsl` | Exact version `2.1.6` | Current non-deprecated TypeScript line with Node 18 support and the guaranteed `schema-dsl/runtime` entry |
...
> `schema-dsl@2.1.6` is the intended target for this release chain; the historical `2.3.x` line exists on npm but has been marked as misreleased/deprecated and must not be followed by upgrades.
...
- `schema-dsl` is fixed to `2.1.6`, `ioredis` to `5.11.1`, and test tooling `mongodb-memory-server` to `10.4.3`.
...
The dependency governance baseline has upgraded and fixed `schema-dsl` from the historical `^1.2.5` to `2.1.6`. The verification standard is as follows:

1. The upstream released the **non-deprecated** 2.x target version on npm: `2.1.6` with Node `>=18`.
2. `npm install schema-dsl@2.1.6 --save-exact` followed by `npm run type-check`.
...
6. This file, Profile, CHANGELOG, and lockfile must be synchronized to `2.1.6`.
