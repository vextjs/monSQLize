...
> **最后更新**: 2025-12-04  
> **当前版本**: v1.2.0 (开发完成) 🎉 | **下一版本**: v1.3.0 (计划中)  
> **综合评分**: **97/100 (A+)** - 企业级质量标准 🎉
...
| 2025-12-04 | **稳定** | **🚀 v1.2.0 开发启动** - 5 个扩展方法（三轮验证通过）🔥 |
| 增长 | **开发中** | v1.2.0 核心扩展方法开发中，预计 4-5 周完成 |
...
- ✅ findAndCount
    - 同时返回数据和总数的便捷方法；1 次调用返回 { data, total }；减少代码量；常用于分页场景。
- ✅ upsertOne
...
#### 扩展方法（v1.2.0 新增）🆕
- ✅ findOneOrCreate **(2024-12-04 新增)**
    - 查询或创建文档的便捷方法；并发安全（E11000 自动重试）；缓存优化（性能提升 59 倍）；代码减少 80%（6-8行 → 1行）。
    - 完美支持 OAuth 登录、标签自动创建等场景；详细文档：[findOneOrCreate.md](./docs/findOneOrCreate.md)
- ✅ safeDelete **(2024-12-04 新增)**
    - 安全删除文档的便捷方法；删除前自动检查依赖关系（防止孤儿数据）；支持软删除用于数据审计；代码减少 80%（10+行 → 3行）。
    - 灵活的 allowCount 配置（允许部分依赖）；防止生产事故；详细文档：[safeDelete.md](./docs/safeDelete.md)
- ✅ updateOrInsert **(2024-12-04 新增)**
    - 深度合并更新或插入的便捷方法；3 种合并策略（replace/shallow/deep）；代码减少 70%（6-8行 → 1-2行）。
    - 完美支持配置管理（只更新指定字段，保留其他所有字段）；真实场景：用户只改 1 个配置，保留 5 个配置；详细文档：[updateOrInsert.md](./docs/updateOrInsert.md)
- ✅ bulkUpsert **(2024-12-04 新增)**
    - 批量 upsert 操作的便捷方法；性能提升 8-41 倍（100条：8.3倍，10000条：41.5倍）；自动分批处理（默认 1000 条/批）。
    - 进度监控（实时回调）；错误恢复（批次失败不影响其他批次）；轻松处理 10 万+ 记录；详细文档：[bulkUpsert.md](./docs/bulkUpsert.md)
