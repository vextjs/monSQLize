...
| 版本 | 发布状态 | 发布日期 | 需求数 | 进度 |
|------|---------|---------|--------|------|
| [v1.2.1](#v121) | ✅ 已完成 | 2026-04-13 | 3 | 3/3 完成 |
...
**发布日期**: 2026-04-13  
**版本类型**: 🐛 Bug 修复：`msq.model()` 实例缓存 + 索引去重  
**进度**: 3 个需求 | 3 个已完成

| 需求标题 | 状态 | 优先级 | 详细 |
|---------|------|--------|------|
| `msq.model()` 实例缓存 + 索引去重 | ✅ 已完成 | P1 | 缓存 Map + redefine 失效标记 |
| 删除 `_createIndexes` 重复定义 | ✅ 已完成 | P2 | 移除死代码 |
| `types/monsqlize.ts` 补全 `model()` / `collection()` | ✅ 已完成 | P2 | TypeScript 类型补全 |
