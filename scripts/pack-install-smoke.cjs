...
try {
    ...
    run(process.execPath, ['-e', "const M=require('monsqlize'); const p=require('monsqlize/package.json'); if(!M || p.version!==process.argv[1]) process.exit(1)", packageJson.version], { cwd: consumerRoot });
    run(process.execPath, ['--input-type=module', '-e', "import M from 'monsqlize'; import {createRequire} from 'node:module'; const p=createRequire(import.meta.url)('monsqlize/package.json'); if(!M || p.version!==process.argv[1]) process.exit(1)", packageJson.version], { cwd: consumerRoot });

    const typeFixture = path.join(consumerRoot, 'consumer.ts');
    fs.writeFileSync(typeFixture, "import MonSQLize, { type DataTaskDefinition } from 'monsqlize';\nconst task: DataTaskDefinition = { name: 'smoke', environment: 'development', target: { collection: 'items' }, steps: [] };\nvoid MonSQLize; void task;\n");
    ...
    console.log(`[pack-install-smoke] passed ${packageJson.name}@${packageJson.version}: CJS, ESM, types, bin, help, version`);
} finally {
