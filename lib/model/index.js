    ...
    _applyTimestamps(opType, method, args) {
        ...
        if (opType === 'insert') {
            ...
            if (Array.isArray(docs)) {
                ...
                args[0] = docs.map(doc => {
                    const newDoc = { ...doc };
                    if (config.createdAt) newDoc[config.createdAt] = now;
                    if (config.updatedAt) newDoc[config.updatedAt] = now;
                    return newDoc;
                });
            } else {
                // insertOne
            ...
            if (method.startsWith('upsert')) {
                ...
                // incrementOne: 暂时跳过时间戳（因为它有特殊的参数验证）
                // TODO: 未来支持 incrementOne 的时间戳
