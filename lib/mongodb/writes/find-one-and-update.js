...
function createFindOneAndUpdateOps(context) {
    ...
    const findOneAndUpdate = async function findOneAndUpdate(filter, update, options = {}) {
        ...
        try {
            // 3. 执行查找并更新操作
            const result = await nativeCollection.findOneAndUpdate(filter, update, options);

            // 4. 自动失效缓存（如果有文档被修改）
            // MongoDB 驱动返回的结果：{ value: <文档或null>, ok: 1, lastErrorObject: {...} }
            ...
            // 6. 返回结果（兼容 MongoDB 驱动的返回格式）
            if (options.includeResultMetadata) {
                return result; // 返回完整元数据 { value, ok, lastErrorObject }
            } else {
                return result.value; // 仅返回文档（默认行为，兼容性更好）
            }
