...
// Bookmark helper
const {bookmarkKey, buildKeyDimsAuto} = require('./find-page');
        ...
        accessor = {
            ...
            explain: async (options = {}) => {
                ...
                try {
                    ...
                    // 记录慢查询日志（如果启用）
                    if (duration > (this.defaults.log.slowQueryMs || 500)) {
                        const safeQuery = this.logger.getSafeShape(query);
                        this.logger.warn(
                            'slow explain query',
                            {
                                ...this.defaults.log.slowQueryTag,
                                collection: collection.collectionName,
                    ...
                    this.logger.error(
                        'explain query failed',
                        {
                            collection: collection.collectionName,
                            database: effectiveDbName,
                            query: this.logger.getSafeShape(query),
                            verbosity,
                        },
                        error
                    );
            ...
            prewarmBookmarks: async (keyDims = {}, pages = []) => {
                if (!self.cache) throw new Error("CACHE_UNAVAILABLE: Cache is required for bookmark operations");
                if (!Array.isArray(pages) || pages.length === 0) throw new Error("INVALID_PAGES: pages must be a non-empty array");

                const results = { warmed: 0, failed: 0, keys: [] };
                for (const page of pages) {
                    if (!Number.isInteger(page) || page < 1) {
                        results.failed++;
                        self.logger.warn(`跳过无效页码: ${page}`);
                        continue;
                    }
                    try {
                        // 使用 jump.step=1 确保 bookmark 被缓存
                        // 注意：这会缓存从 page 1 到目标页的所有中间页
                        const result = await accessor.findPage({ ...keyDims, page, totals: false, jump: { step: 1 } });
                        // 检查是否返回了数据，如果为空则视为失败（超出范围）
                        if (result.items && result.items.length > 0) {
                            results.warmed++;
                        } else {
                            results.failed++;
                            self.logger.warn(`页面 ${page} 无数据（可能超出范围）`);
                        }
                    } catch (error) {
                        results.failed++;
                        self.logger.error(`预热页面 ${page} 失败:`, error);
                    }
                }
                const ns = `${instanceId}:${self.type}:${effectiveDbName}:${collection.collectionName}`;
                results.keys = await self.cache.keys(`${ns}:bm:*`);
                return results;
            },
            ...
            listBookmarks: async (keyDims) => {
                ...
                const keys = await self.cache.keys(pattern);
                const pages = keys.map((key) => {
                    const match = key.match(/:(\d+)$/);
                    return match ? parseInt(match[1], 10) : null;
                }).filter((page) => page !== null).sort((a, b) => a - b);
            ...
            clearBookmarks: async (keyDims) => {
                ...
                const keysBefore = await self.cache.keys(pattern);
                const cleared = await self.cache.delPattern(pattern);
