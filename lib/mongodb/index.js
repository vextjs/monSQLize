...
module.exports = class {
    ...
    collection(databaseName, collectionName) {
        ...
        return {
            ...
            /**
             * 查询多条记录
             * @param {Object} [options={}] - { query, projection, sort, limit, skip, cache, maxTimeMS, stream }
             * @returns {Promise<Array>|ReadableStream} 记录数组或可读流（当 stream: true 时）
             */
            find: (options = {}) => {
                options.projection = normalizeProjection(options.projection);
                const {
             ...
             * @returns {Promise<Array>|ReadableStream} 聚合结果数组或可读流（当 stream: true 时）
             */
