...
function createFindPage(ctx) {
    ...
    return async function findPage(options = {}) {
        ...
        if (options.stream) {
            ...
            if (Number.isInteger(page) && page > 1) {
                const err = new Error('流式模式不支持跳页功能，请使用 after/before 游标分页');
                err.code = 'STREAM_NO_JUMP';
                throw err;
            }

            // 流式模式不支持 totals
            if (options.totals?.mode && options.totals.mode !== 'none') {
                const err = new Error('流式模式不支持 totals 统计');
                err.code = 'STREAM_NO_TOTALS';
                throw err;
            }

            // 流式模式不支持 explain
            if (options.explain) {
                const err = new Error('流式模式不支持 explain 分析，请使用普通分页模式');
                err.code = 'STREAM_NO_EXPLAIN';
                throw err;
            }
    ...
    async function attachTotals({ collection, cache, ns, keyDims, limit, query, totals }) {
        ...
        if (mode === 'sync') {
            const result = await computeAndCacheTotal({ collection, cache, ns, keyDims, query, limit, hint: totals.hint, collation: totals.collation, maxTimeMS: totals.maxTimeMS ?? 2000, ttlMs, logger });
