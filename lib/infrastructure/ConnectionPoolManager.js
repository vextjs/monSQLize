...
class ConnectionPoolManager {
     ...
     * @returns {{name: string, client: MongoClient}}
     * @throws {Error} 如果无可用连接池
     */
    selectPool(operation, options = {}) {
        // 手动指定连接池
        if (options.pool) {
            const client = this._getPool(options.pool);
            if (!client) {
                throw new Error(`Pool '${options.pool}' not found`);
            }
            return { name: options.pool, client };  // 🔴 修复问题3：返回 {name, client}
        }
        ...
        const client = this._getPool(poolName);
        if (!client) {
