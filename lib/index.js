...
module.exports = class {
    ...
    constructor(options) {
        ...
        this.cacheAutoInvalidate = cacheConfig.autoInvalidate !== undefined
 ...
 */
    async connect() {
        ...
        try {
            this._connecting = (async () => {
                ...
                if (this._poolsConfig && Array.isArray(this._poolsConfig) && this._poolsConfig.length > 0) {
    ...
    async addPool(config) {
        if (!this._poolManager) {
            throw new Error('Multi-pool is not enabled. Configure pools in constructor options.');
        }
    ...
    collection(collectionName) {
        if (!this.dbInstance) {
            const err = new Error('Database is not connected. Call connect() before accessing collections.');
            err.code = 'NOT_CONNECTED';
            throw err;
        }
    ...
    async _loadModels() {
        ...
        if (typeof this._modelsConfig === 'string') {
            ...
            recursive = false;
        } else if (typeof this._modelsConfig === 'object' && this._modelsConfig.path) {
            // 完整配置
        ...
        const globPattern = pattern.replace(/\./g, '\\.').replace(/\*/g, '.*').replace(/\{([^}]+)\}/g, '($1)').replace(/,/g, '|');
        ...
        try {
            ...
            this.logger.error(`[Model] Failed to scan directory: ${absolutePath}`, err);
            return;
        }
        ...
        for (const file of files) {
            try {
                ...
                // 验证 Model 定义
                if (!modelDef) {
                    this.logger.warn(`[Model] Skipping ${file}: export is null or undefined`);
                    errorCount++;
                    continue;
                }
                ...
                if (typeof modelDef.name !== 'string' || modelDef.name.trim() === '') {
                    this.logger.warn(`[Model] Skipping ${file}: 'name' must be a non-empty string`);
                    errorCount++;
                    continue;
                }
                ...
                if (Model.has(modelDef.name)) {
                    this.logger.debug(`[Model] Model '${modelDef.name}' already registered, skipping ${file}`);
                    continue;
                }
                ...
                if (this.logger) {
                    this.logger.debug(`[Model] ✅ Loaded: ${modelDef.name} from ${path.relative(process.cwd(), file)}`);
                }
            } catch (err) {
                errorCount++;
                if (this.logger) {
                    this.logger.error(`[Model] ❌ Failed to load ${path.relative(process.cwd(), file)}:`, err.message);
                }
            }
        }
